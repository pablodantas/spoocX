...
import { getWingsAddress, getLotteryAddress } from 'utils/addressHelpers'
...
export const useAllowance = () => {
  ...
  const wingsContract = useERC20(getWingsAddress())
  const lotteryAddress = getLotteryAddress()

  useEffect(() => {
    const fetchAllowance = async () => {
      const res = await wingsContract.methods.allowance(account, lotteryAddress).call()
      setAllowance(new BigNumber(res))
    }
    if (account && wingsContract && lotteryAddress) {
      fetchAllowance()
    }
  }, [account, wingsContract, lotteryAddress, fastRefresh])
