...
import { Flex, Link } from 'jetswap-uikit-new'
...
const IfoCardTime: React.FC<IfoCardTimeProps> = ({
  status,
  /* secondsUntilStart, secondsUntilEnd, */ block,
  blockToEnd,
  blocksRemaining,
  ...
  return (
    <Details>
      ...
      <Link
        href={`https://bscscan.com/block/countdown/${status === 'coming_soon' ? block : blockToEnd}`}
        target="blank"
        rel="noopener noreferrer"
        ml="8px"
      >
        {blockCount} blocks {status === 'coming_soon' ? 'until start' : 'until finish'}
      </Link>
    </Details>
  )
}
