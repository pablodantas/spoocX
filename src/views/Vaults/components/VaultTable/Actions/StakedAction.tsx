...
import { Button, useModal, IconButton, AddIcon, MinusIcon } from 'jetswap-uikit-new'
...
import { ActionContainer, ActionTitles, ActionContent, Earned, Title, Subtle, Staked as Balanced } from './styles'
...
const Staked: React.FunctionComponent<VaultWithStakedValue> = ({
  ...
  const isApproved = lpSymbol === 'BNB' ? true : account && allowance && allowance.isGreaterThan(0)
  ...
  if (props.isSingle) {
    lpPrice = new BigNumber(prices[lpSymbol === 'BNB' ? 'wbnb' : lpSymbol.toLowerCase()])
  } else {
      ...
      <ActionContainer>
        ...
        <ActionContent>
          <Button width="100%" onClick={onPresentDeposit} variant="secondary">
            ...
            lpSymbol === 'BTCB' ||
            lpSymbol === 'ETH'
              ? t(`Deposit %symbol%`, { symbol: lpSymbol })
              : t('Deposit LP')}
          </Button>
        </ActionContent>
      </ActionContainer>
