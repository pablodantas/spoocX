...
const Vaults: React.FC = () => {
  ...
  const autoPrice = usePriceAutoBusd()
  ...
  const calculateAPY = (vault) => {
    ...
    let apy
    if (
      (vault.provider === 'Fortress' || vault.provider === 'Venus') &&
      (vault.lpSymbol === 'BNB' || vault.lpSymbol === 'BTCB' || vault.lpSymbol === 'ETH')
    ) {
      apy = vault.apy
    } else {
      let rewardPrice = wingsPrice
      if (vault.provider === 'Pancake') {
        rewardPrice = cakePrice
      } else if (vault.provider === 'autofarm') {
        rewardPrice = autoPrice
      } else if (vault.provider === 'Apeswap') {
        rewardPrice = prices.banana
      }
      apy = getVaultApy(vault.poolWeight, rewardPrice, totalFarmLiquidity)
    }
    return apy
  }
  ...
  const vaultsList = useCallback(
    (vaultsToDisplay: Vault[]): VaultWithStakedValue[] => {
      let vaultsToDisplayWithAPY: VaultWithStakedValue[] = vaultsToDisplay.map((vault) => {
        ...
        if (vault.lpSymbol === 'WINGS' || vault.lpSymbol === 'CAKE' || vault.lpSymbol === 'BANANA') {
        ...
        if (vault.lpSymbol === 'BNB' || vault.lpSymbol === 'BTCB' || vault.lpSymbol === 'ETH') {
          apy = vault.apy
          totalLiquidity = new BigNumber(vault.vaultBalance).times(quoteTokenPriceUsd)
        } else {
          let rewardPrice = prices.wings
          if (vault.provider === 'Pancake') {
            rewardPrice = prices.cake
          } else if (vault.provider === 'autofarm') {
            rewardPrice = prices.auto
          } else if (vault.provider === 'Apeswap') {
            rewardPrice = prices.banana
          }
          apy = getVaultApy(vault.poolWeight, rewardPrice, totalFarmLiquidity, vault.provider)
        }
        ...
        <FlexLayout>
          <Route exact path={`${path}`}>
            {vaultsStaked.map((vault) => (
              <VaultCard key={vault.pid} vault={vault} wingsPrice={wingsPrice} account={account} removed={false} />
            ))}
          </Route>
          <Route exact path={`${path}/history`}>
            {vaultsStaked.map((vault) => (
              <VaultCard key={vault.pid} vault={vault} wingsPrice={wingsPrice} account={account} removed />
            ))}
          </Route>
        </FlexLayout>
      </div>
