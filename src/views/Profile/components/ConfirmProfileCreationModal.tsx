...
import { Modal, Flex, Text } from 'jetswap-uikit-new'
import { useDispatch } from 'react-redux'
import BigNumber from 'bignumber.js'
import { useTranslation } from 'contexts/Localization'
import { useWings, usePancakeRabbits, useProfile } from 'hooks/useContract'
...
const ConfirmProfileCreationModal: React.FC<Props> = ({
  ...
  const {
    ...
    onRequiresApproval: async () => {
      try {
        const response = await wingsContract.methods.allowance(account, profileContract.options.address).call()
    ...
    onApprove: () => {
      return wingsContract.methods.approve(profileContract.options.address, allowance.toJSON()).send({ from: account })
    },
    onConfirm: () => {
