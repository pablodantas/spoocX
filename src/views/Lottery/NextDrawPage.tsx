...
import { Button, BaseLayout } from 'jetswap-uikit-new'
...
const NextDrawPage: React.FC = () => {
  ...
  const lotteryTotalPrizeData = useLotteryTotalPrizeData(currentLotteryNumber)

  useEffect(() => {
    const fetchRecentLotteryData = async () => {
      try {
        const data = await fetchLotteryTicketData(account, currentLotteryNumber)
  ...
  }, [currentLotteryNumber, account, setLotteryTicketData, fastRefresh])
    ...
    try {
      ...
      await Promise.all(
        lotteryTotalPrizeData.rewardTicketData.map(async (data) => {
          ...
          await claimReward(lotteryContract, lotteryId, ticketIds, account)
        }),
      )
    } catch (e) {
      console.error(e)
  ...
  }, [setRequestedClaim, lotteryTotalPrizeData, account, lotteryContract])
