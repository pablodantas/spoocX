...
import { Button, Modal } from 'jetswap-uikit-new'
...
const BuyTicketModal: React.FC<BuyTicketModalProps> = ({
  ...
  const getCostWithDiscount = useCallback(async () => {
    const res = await fetchDiscountData()
    setDiscountData(res)
  }, [])
  ...
  const handleBuy = useCallback(async () => {
    try {
      ...
      const txHash = await onBuyTickets(val)
  ...
  }, [onBuyTickets, setRequestedBuy, val])
  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        max={fullBalance}
        symbol={t('TICKET')}
        availableSymbol="WINGS"
      />
      <Wrap>
        <div>
          <div>
            <Tips>{t(`1 Ticket = %lotteryPrice% WINGS`, { lotteryPrice: ticketPrice })}</Tips>
          </div>
          <div>
            <Announce>
              {t('Ticket purchases are final. Your WINGS cannot be returned to you after buying tickets.')}
            </Announce>
          </div>
        </div>
        <div>
          <StyledMaxText>{t(`%num% WINGS Available`, { num: fullBalance.toLocaleString() })}</StyledMaxText>
        </div>
      </Wrap>
      <Final>
        <FinalContent>{t(`You will spend: %num% WINGS`, { num: cakeCosts(val).toLocaleString() })}</FinalContent>
      </Final>
      <ModalActions>
