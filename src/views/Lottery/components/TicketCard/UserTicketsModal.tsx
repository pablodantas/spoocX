...
const UserTicketsModal: React.FC<UserTicketsModalProps> = ({ myTicketNumbers, lotteryNo = 0, onDismiss }) => {
  const currentLotteryNumber = useLotteryCurrentRoundNo()
  const [winNumbers, setWinNumbers] = useState([0, 0, 0, 0])
  const [lotteryStatus, setLotteryStatus] = useState(0)
  const [listItems, setListItems] = useState<any>(<></>)
  const lotteryInfo = useLotteryInfo(lotteryNo > 0 ? lotteryNo : currentLotteryNumber)
