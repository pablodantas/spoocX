...
const CakeWalletBalance = () => {
  ...
  const busdBalance = new BigNumber(getBalanceNumber(wingsBalance)).multipliedBy(usePriceWingsBusd()).toNumber()
  const { account } = useWeb3React()

  if (!account) {
    return (
      <Text fontSize="36px" color="#fff" style={{ lineHeight: '54px' }}>
        {t('Locked')}
      </Text>
    )
  }
